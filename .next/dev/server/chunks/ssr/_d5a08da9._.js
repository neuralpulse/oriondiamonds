...
function EarringsPage() {
    ...
    const transformEarringsData = async (productsEdges)=>{
        // First, transform the data with price calculation
        const transformed = await Promise.all(productsEdges.map(async ({ node: product })=>{
            ...
            console.log(`Product: ${product.title}, Calculated Price: ${calculatedPrice}`);
            return {
    ...
    const calculateProductPrice = async (description, selectedKarat = "10K")=>{
        try {
            ...
            console.log("Diamond Shapes:", diamondShapes);
            console.log("Diamond Counts:", diamondCounts);
            console.log("Diamond Weights:", diamondWeights);
...
//# sourceMappingURL=_d5a08da9._.js.map