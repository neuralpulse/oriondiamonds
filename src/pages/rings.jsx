...
export function Rings() {
  ...
  const fetchRings = async () => {
    try {
      ...
      if (response.data?.collection?.products?.edges) {
        const transformedRings = transformRingsData(
  ...
  const transformRingsData = (productsEdges) => {
    return productsEdges.map(({ node: product }) => {
      ...
      return {
        ...
        allVariants: product.variants.edges.map((v) => v.node), // Include all variants if needed
      };
    });
  };
